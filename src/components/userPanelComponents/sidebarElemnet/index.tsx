...
const SidebarElement = ({text, logo, link}: sideBarElementType) => {
    return(
        <Link
          href={`${link}`}
          className="cursor-pointer group hover:bg-white w-full py-4 flex items-center ps-6 lg:rounded-s-3xl rounded-3xl  hover:ease-in-out hover:duration-150 gap-2"
        >
          {logo}
